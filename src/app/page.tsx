...
async function getImageUrl(page: any): Promise<string | null> {
  const attachments = page.properties?.Attachment?.files ?? [];
...
export default async function Home() {
  ...
  const posts = await Promise.all(
    pages.map(async (page: any) => {
      ...
      const estado =
        page.properties?.Estado?.status?.name ??
        page.properties?.Estado?.select?.name ?? "";
      const pinned = page.properties?.["Nota Importante"]?.rich_text?.[0]?.plain_text === "pin";
  ...
  return (
    <main style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", maxWidth: "480px", margin: "0 auto", background: "#fff", minHeight: "100vh" }}>
      ...
      {/* TABS */}
      <div style={{ display: "flex", justifyContent: "center", gap: "32px", padding: "10px 0", borderBottom: "1px solid #dbdbdb" }}>
